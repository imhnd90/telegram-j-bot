...
async def filter_and_forward(event):
    ...
    # 1. ANTI-SPAM FILTER
    if any(promo in combined_search_text for promo in PROMO_KEYWORDS):
    ...
    if any(job in combined_search_text for job in JOB_KEYWORDS):
        ...
        # 🌟 FIX: If it's a real attached image file, forward it
        if event.message.media and isinstance(event.message.media, MessageMediaPhoto):
            ...
            # 🌟 FIX: Explicitly set link_preview=False to block external website preview boxes from appearing!
